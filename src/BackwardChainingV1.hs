...
flaggedFacts :: Expr -> [Expr] -> [Expr]
exprFactsFlagged e flagged =
  let es = getFactsInExpr e in es `intersect` flagged

--| Return the facts of the Expr e that are in the list flagged
flaggedFactsInExpr :: Expr -> [(Expr, State)] -> [(Expr, State)]
flaggedFactsInExpr e flagged =
  let es = getFactsInExpr e
  in flaggedFacts es flagged

--| Return the facts of the List fs that are in the list flagged
flaggedFacts :: [Expr] -> [(Expr, State)] -> [(Expr, State)]
flaggedFacts fs flagged =
  let searchFact tab (x, _) = isJust (elem x tab)
  in filter (searchFact fs) flagged
